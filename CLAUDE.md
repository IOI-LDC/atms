...
**Failure classification (`is_failure`):** nullable boolean on corrective MRs — `true`/`false` = classified, `null` = not yet reviewed. Set (required) at MR approval via `POST /maintenance-requests/{id}/approve` (422 if missing for a corrective MR in `pending_review`); optionally overridden at WO closure via `POST /work-orders/{id}/close` for corrective-origin WOs only (key absent = keep the review-time value; never send `null`, which would clobber it). Preventive MRs are never classified. Drives the Dashboard **MTBF** and **Failure Rate** KPIs (both now count `is_failure = true`, not every corrective event; MTTR unchanged). Exposed on `MaintenanceRequestResource` (always) and inside the **partial** `maintenance_request` embed on `WorkOrderResource` (`{ id, number, is_preventive, is_failure }` — carries `is_preventive`, not `type`; corrective-origin ⇔ `is_preventive === false`). Renamed wire-level from `is_fault` on 2026-07-05 ("Failure" is the correct reliability term; migration recreated, no deprecation window).
...
**Decision (2026-07-04, confirmed 2026-07-11):** production transport is **Microsoft Graph `sendMail`** (OAuth2 client credentials), sending from `notification@ldc.com.ly`. SMTP AUTH is **ruled out** (LDC M365 tenant disables it — `SmtpClientAuthenticationDisabled`, verified `535 5.7.139`). Power Automate is retired and has been removed. `GraphAccountEmailTransport` is selected by `ACCOUNT_EMAIL_TRANSPORT=graph`; the fake transport remains for development and tests. It caches the app-only access token, retries Graph `429` responses using `Retry-After`, and activation/reset notifications share a queue lock. Config: `GRAPH_TENANT_ID`, `GRAPH_CLIENT_ID`, `GRAPH_CLIENT_SECRET`, `GRAPH_MAILBOX` — a **separate Entra app** from the `LDC_ERP_*` (Dynamics 365 BC) app. Full design + Azure provisioning + secret/certificate expiry & renewal: `docs/03-backend/NOTIFICATIONS.md`.
...
- **Email transport: Microsoft Graph `sendMail` is the only production transport.** Power Automate is removed; Graph is wired for activation and password-reset emails, with a token cache, shared queue lock, and 429 `Retry-After` retry. Phase 1 scope excludes operational MR/WO emails. `ACCOUNT_EMAIL_TRANSPORT` accepts `fake` and `graph`. See `docs/03-backend/NOTIFICATIONS.md`.
