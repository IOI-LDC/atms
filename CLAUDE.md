...
- **Closing a WO does six things, in this order** (`CloseWorkOrder`): status transition + audit → asset operational-status transition → `is_failure` override → **confirms the readings taken on that WO** → optional **declared service** → **meter snapshot**, then the PM branch stamps `last_triggered_*` and cascades a reset to lower PM levels (L1–L4).
  - ⚠️ **That ordering is load-bearing.** The PM branch queries the *latest confirmed reading* in **two** places — the assignment's own baseline and again inside `resetLowerLevelAssignments()`. Confirmation and the declared service must run before both, or a reading confirmed by this same close silently misses the baseline. The snapshot must run after them, or it captures the previous meter position.
  - **Reading confirmation is a by-product of closing, not a button.** There is no verify action in the UI and none should be added — close is Admin/Manager only, so the closer is a second pair of eyes over the technician who took the reading. `ConfirmWorkOrderReadings` confirms oldest-first (`ConfirmMeterReading` rejects a reading dated before the latest confirmed one, so newest-first would strand the rest) and **skips** any reading that still fails a guard, auditing it as `meter_reading.confirm_skipped`. A data-quality problem must never block an operational transition.
  - **Declared service** (`serviced_pm_assignment_id`) covers the repair-shop case: the asset was in for a repair, a PM level was due, the team did both. It resets that assignment's baselines, cascades down, and cancels any pending PM request with suppression `decision_type = 'performed_under_repair'` — not `cancelled`, so compliance reporting does not read it as a skipped service.
  - **`work_order_meter_snapshots`** records the meter position **per reading type** at close, which is what makes "usage since the last repair" answerable. Immutable: never recomputed when a source reading is later edited.
