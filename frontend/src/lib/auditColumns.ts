...
export const auditEventGroups: AuditEventGroup[] = [
  ...
  literalGroup('Work Order Form', [
    ...
    // Bulk save from the checklist sheet — one entry per Save, before/after
    // keyed by field id. Distinct from the singular event above, which the
    // retained single-field PATCH still writes.
    'work_order_form.field_values_updated',
