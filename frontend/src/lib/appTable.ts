 ...
 *                    identity key). Dot notation reads a nested property
 *                    (`asset.serial_number`), which is how a column derived
 *                    from an embedded object still gets its own sorting and
 *                    global search. The `#cell` slot receives the full typed
 ...
 *                    the identity badges) so the search matches
