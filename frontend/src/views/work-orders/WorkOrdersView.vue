...
const activeSlice = computed(() => {
  if (activeTab.value === 'pending-approval')
    return {
      rows: awaiting.rows.value,
      loading: awaiting.loading.value,
      load: awaiting.load,
      emptyText: 'No requests awaiting approval.',
      label: 'Requests awaiting approval',
    }
  if (activeTab.value === 'all-requests')
    return {
      rows: allRequests.rows.value,
      loading: allRequests.loading.value,
      load: allRequests.load,
      emptyText: 'No maintenance requests found.',
      label: 'All maintenance requests',
    }
  return {
    rows: myRequests.rows.value,
    loading: myRequests.loading.value,
...
<template>
    ...
    <div class="page-section">
      <div class="page-header">
        <div class="page-heading">
          ...
          <p class="page-subtitle">
            Submit and track corrective and preventive maintenance requests
          </p>
        </div>
      ...
      >
        ...
        <template #cell="{ column, row, value }">
          ...
          <AssetIdentity v-else-if="column.field === 'asset'" :asset="row.asset" stacked />
  ...
  >
    <SheetContent side="right" class="create-sheet">
      ...
      <div class="create-sheet-footer">
        ...
        <Button :disabled="createLoading || !selectedAsset" @click="requestCreate"
  ...
  <Dialog v-model:open="confirmCreateOpen">
    <DialogContent>
      ...
      <DialogFooter>
        <Button variant="outline" :disabled="createLoading" @click="confirmCreateOpen = false"
          >Back</Button
        >
