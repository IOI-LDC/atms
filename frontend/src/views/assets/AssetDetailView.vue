...
// SelectItem carries an empty-string value, because it reserves '' for clearing.
              ...
              <div class="detail-card-content">
                <div class="detail-grid">
                  ...
                  <div class="detail-field">
                    ...
                    <p class="detail-field-value">
                      <span v-if="record.erp_asset_code" class="atms-erp-code">{{
                        record.erp_asset_code
                      }}</span>
                      <span v-else>—</span>
                    </p>
                  </div>
                  <div class="detail-field">
                    <span class="detail-field-label">FA Asset Class</span>
                    <p class="detail-field-value">
    ...
    >
      <SheetContent side="right" class="create-sheet">
        <SheetHeader class="create-sheet-header">
          ...
          <SheetDescription>
            Update operational details for {{ record?.name }}. FA Asset Class is managed by ERP and
        ...
        <div class="create-sheet-body">
          ...
          <div class="form-grid">
            ...
            <!-- Size — controlled list of sizes already in use -->
            <div class="form-field">
              <Label for="edit-size">Size <span class="field-optional">— optional</span></Label>
              <Select
                :model-value="draft.size_inches || '__none__'"
                @update:model-value="
                  (v) => {
                    const s = String(v)
                    draft.size_inches = s === '__none__' ? '' : s
                  }
                "
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem v-for="s in assetSizes" :key="s.value" :value="String(s.value)">
                    {{ s.label }}
                  </SelectItem>
                </SelectContent>
              </Select>
              <p v-if="!assetSizesLoading && assetSizes.length === 0" class="form-help">
                No sizes are available yet — they arrive with the asset and parts data import.
              </p>
            ...
            <!-- Maintenance Category — import-managed vocabulary, assignment only -->
            <div class="form-field">
              <Label for="edit-maint-category">
                Maintenance Category <span class="field-optional">— optional</span>
              </Label>
              <Select v-model="maintenanceCategoryValue">
                <SelectTrigger id="edit-maint-category">
                  <SelectValue placeholder="No category" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none__">No category</SelectItem>
                  <SelectItem v-for="c in maintenanceCategories" :key="c.id" :value="String(c.id)">
                    {{ c.name }}
                  </SelectItem>
                </SelectContent>
              </Select>
              <p
                ...
                class="form-help"
              >
                No categories are available yet — they arrive with the asset and parts data import.
              </p>
