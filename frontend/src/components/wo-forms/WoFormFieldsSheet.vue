...
watch(() => props.template, (t) => { rows.value = draftsFrom(t?.fields) }, { immediate: true })
...
function toggleFlag(row: FieldDraft, key: 'has_pre_post' | 'is_required', value: boolean | 'indeterminate') {
  row[key] = value === true
  commitUpdate(row)
}
...
watch(() => props.open, (nowOpen) => { if (nowOpen) resetAddDraft() })
watch(() => props.addedTick, () => resetAddDraft())

function handleAdd() {
  addError.value = ''
  if (!newLabel.value.trim()) { addError.value = 'Label is required.'; return }
  emit('add', {
  ...
  <Dialog :open="open" @update:open="(v) => !v && emit('close')">
    <DialogContent class="wo-form-fields-dialog">
      ...
      <template v-else-if="template">
        ...
        <div v-if="rows.length === 0" class="empty-state">No fields yet — add the first one below.</div>
          ...
          <div v-for="(row, i) in rows" :key="row.id" class="wo-form-field-row">
            <div class="wo-form-field-row-grid">
              <div class="form-field">
                ...
                <Input :id="`wo-field-label-${row.id}`" v-model="row.label" @blur="commitUpdate(row)" />
              </div>
              <div class="form-field">
                <Label :for="`wo-field-type-${row.id}`">Type</Label>
                <Select :model-value="row.field_type" @update:model-value="(v) => setFieldType(row, String(v))">
                  <SelectTrigger :id="`wo-field-type-${row.id}`"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem v-for="t in FIELD_TYPES" :key="t" :value="t">{{ woFormFieldTypeLabel(t) }}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div v-if="row.field_type === 'numeric'" class="form-field">
                <Label :for="`wo-field-unit-${row.id}`">Unit</Label>
                <Input :id="`wo-field-unit-${row.id}`" v-model="row.unit" placeholder="E.g. PSI" @blur="commitUpdate(row)" />
              </div>
            </div>

            <div class="wo-form-field-row-flags">
              <Label class="checkbox-field">
                <Checkbox :model-value="row.has_pre_post" @update:model-value="(v) => toggleFlag(row, 'has_pre_post', v)" />
                <span>Pre + post values</span>
              </Label>
              <Label class="checkbox-field">
                <Checkbox :model-value="row.is_required" @update:model-value="(v) => toggleFlag(row, 'is_required', v)" />
                <span>Required</span>
              </Label>
            </div>
            ...
            <div class="wo-form-field-row-footer">
              <div class="wo-form-field-order-actions">
                <Button variant="ghost" size="icon-sm" :disabled="i === 0" :aria-label="`Move ${row.label} up`" @click="moveUp(i)">
                  <ChevronUpIcon />
                </Button>
                <Button variant="ghost" size="icon-sm" :disabled="i === rows.length - 1" :aria-label="`Move ${row.label} down`" @click="moveDown(i)">
                  <ChevronDownIcon />
                </Button>
              </div>
              <Button variant="ghost" size="icon-sm" :aria-label="`Remove ${row.label}`" @click="openDelete(row)">
                <Trash2 />
              </Button>
            </div>
          </div>
    ...
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription v-if="deleteTarget">
          Remove "{{ deleteTarget.label }}" from this template? Existing work order snapshots keep their captured values — only new snapshots stop including this field.
        </DialogDescription>
      </DialogHeader>
      <DialogFooter>
