...
function choose(asset: Asset) {
  // The trigger is a plain text input, so the selected value must be a string.
  // Only the name goes in it — serial, size and category are shown as badges
  // beside the input, never concatenated into the label.
  model.value = { id: asset.id, label: asset.name }
  open.value = false
}
      ...
      >
        ...
        <template v-else-if="results.length > 0">
          ...
          >
            ...
            <AssetIdentityBadges :asset="asset" />
          </Button>
        </template>
