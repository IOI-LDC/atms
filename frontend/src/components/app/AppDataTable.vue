...
// `accessorKey` reads a top-level property, or a nested one when `field` uses
// dot notation (`asset.serial_number`) — TanStack resolves the path, which is
// what gives such a column its own sorting and global search. A whole object
// prop (`asset`, `current_location`) is instead returned intact and rendered
// from `row` in the slot. Sorting/filtering map 1:1, and a custom `comparator`
// is adapted to TanStack's `sortingFn` signature.
...
// column also searches its identity badge values, which it renders but
