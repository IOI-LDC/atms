...
const props = withDefaults(
  defineProps<{
    ...
    /**
     * Restrict results to parts compatible with this asset. Omit for an
     * unfiltered catalogue search.
     */
    compatibleWithAssetId?: number | null
  }>(),
  {
    disablePortal: true,
    compatibleWithAssetId: null,
  },
)
...
const { query, results, busy, search, loadInitial, reset } = usePartSearch(
  toRef(props, 'compatibleWithAssetId'),
)
...
function choose(part: Part) {
  // Name only in the text input; part number, size and category render as
  // badges beside it rather than being folded into the label.
  model.value = { id: part.id, label: part.name }
  open.value = false
}
      ...
      >
        ...
        <template v-else-if="results.length > 0">
          ...
          >
            ...
            <PartIdentityBadges :part="part" show-stock />
          </Button>
        </template>
        <div v-else class="asset-combobox-empty">
          <template v-if="displayQuery.trim()">No parts match your search.</template>
          <template v-else-if="props.compatibleWithAssetId"
            >No parts are compatible with this asset.</template
          >
          <template v-else>No parts available.</template>
        </div>
      </div>
    </PopoverContent>
  </Popover>
