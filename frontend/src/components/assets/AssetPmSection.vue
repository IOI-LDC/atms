...
/** pm_rule_id → assignment is_active, for templates already on this asset. */
const assignedRules = ref<Map<number, boolean>>(new Map())
const selectedTemplate = ref<string>('')
const assignError = ref('')

/** Templates still assignable — an asset can hold each template only once. */
const assignableCount = computed(
  () => templates.value.filter((t) => !assignedRules.value.has(t.id)).length,
)

function assignedNote(templateId: number): string {
  const isActive = assignedRules.value.get(templateId)
  if (isActive === undefined) return ''
  return isActive ? ' — already assigned' : ' — already assigned (inactive)'
}
...
async function openAssign() {
  ...
  const [activeTemplates, assigned] = await Promise.all([
    loadActiveTemplates(),
    loadAssignedRuleStates(props.assetId),
  ])
  templates.value = activeTemplates
  assignedRules.value = assigned
...
async function confirmAssign() {
  ...
  if (result.ok) {
    ...
    assignError.value = result.message ?? 'Failed to assign template.'
  }
}
          ...
          <Select v-model="selectedTemplate">
            ...
            <SelectContent disable-portal>
              <SelectItem
                ...
                :key="t.id"
                :value="String(t.id)"
                :disabled="assignedRules.has(t.id)"
              >
                {{ t.name }}{{ assignedNote(t.id) }}
              </SelectItem>
            </SelectContent>
          </Select>
