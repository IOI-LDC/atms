...
/**
 * The one shape every embedded asset reference uses, mirroring the backend
 * AssetIdentityResource. Rendered exclusively through <AssetIdentity>: name as
 * normal text, then value-only badges for Serial Number, Size and Maintenance
 * Category. `erp_asset_code` is intentionally absent — it is Admin-only and
 * lives on the full Asset detail payload.
 */
export interface AssetIdentity {
  ...
  /** O&G fractional display, e.g. `6 3/4"`. */
  size: string | null
  /** Exact canonical inches for matching, e.g. `6.75000`. */
  size_inches: string | null
  maintenance_category?: MaintenanceCategoryOption | null
}

/** Mirrors the backend PartIdentityResource. See AssetIdentity. */
...
export interface Asset {
  ...
  /** ERP FA subclass under its display name. Separate from maintenance_category. */
  ...
  serial_number: string | null
  /** O&G fractional display, e.g. `6 3/4"`. Null when the asset has no size. */
  size: string | null
  /** Exact canonical inches for matching/submission, e.g. `6.75000`. */
  size_inches: string | null
  /** Controlled Maintenance Category — distinct from `fa_subclass_code`. */
  maintenance_category?: MaintenanceCategoryOption | null
  model: string | null
...
export interface Part {
  ...
  /** O&G fractional display, e.g. `6 3/4"`. */
  size: string | null
  /** Exact canonical inches, e.g. `6.75000`. */
  size_inches: string | null
  maintenance_category?: MaintenanceCategoryOption | null
  available_quantity: number
  is_active: boolean
...
/**
 * A controlled Maintenance Category. Local ATMS data, unrelated to the ERP
 * `fa_subclass_code` (FA Asset Class). The vocabulary is managed by Admins
 * (and seeded by the workbook import); assets and parts pick from it.
 *
 * `is_active` is only returned by the admin list endpoint — the public
 * dropdown endpoint (`/list-options/maintenance_categories`) returns active
 * rows without the flag.
 */
...
export interface MtbfRow {
  group_key: string | number | null
  group_label: string | null
  /** Present only when group_by=asset — shared identity for rendering. */
  asset?: AssetIdentity | null
