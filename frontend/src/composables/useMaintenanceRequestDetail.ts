...
export function useMaintenanceRequestDetail() {
  ...
  // Attachments stay open at every workflow stage — pending_review, converted,
  // rejected and cancelled alike. Workflow fields lock on conversion but the
  // evidence trail does not, so this deliberately does NOT gate on isPending.
  // The creator qualifies whatever role they hold. Backend policy is
  // authoritative; this only decides whether to offer the control.
  const canUploadAttachment = computed(
    () => !!record.value && (auth.isAdminOrManager || isOwnRequest.value),
  )
  ...
  async function doUpload() {
    ...
    try {
      for (const file of uploadFiles.value) {
        ...
        await api.upload(`/maintenance-requests/${record.value.id}/attachments`, form)
      }
      toast.success(
