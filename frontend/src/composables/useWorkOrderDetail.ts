 ...
 *  GET    /list-options/usage_reading_types  -> { data: Type[] }   (active-only, any role)
...
export function useWorkOrderDetail() {
  ...
  async function loadAssetReadings() {
    ...
    // The readings table renders the type *name*, which only lives in the
    // catalogue — without this it falls back to a generic "Meter reading"
    // label until the user happens to open one of the dialogs.
    void ensureReadingTypes()
  }
  ...
  async function ensureReadingTypes() {
    ...
    try {
      // Read via the auth-only list-options path, not the Administrator-gated
      // /admin/usage-reading-types: the Record-reading dialog is open to
      // Managers and the assigned Technician too, and the admin route 403s for
      // them, leaving the type picker empty. Returns active types only.
      const res = await api.get<{ data: { id: number; name: string; unit: string }[] }>(
        '/list-options/usage_reading_types',
      )
