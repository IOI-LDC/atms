...
export function useDashboard() {
  ...
  const showPendingMr      = computed(() => data.value?.summary.pending_maintenance_requests !== undefined)
  ...
  const showRecentlyClosed = computed(() => data.value?.summary.recently_closed_work_orders !== undefined)
  ...
  const hasActionRequired = computed(() => showPendingMr.value || showOpenWo.value || showOverduePm.value)
