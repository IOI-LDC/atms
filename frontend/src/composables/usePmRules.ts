...
/** Result of assigning a template to an asset. */
export interface AssignRuleResult {
  ok: boolean
  data?: AssetPmAssignment
  /** Server message — carries the 409 "already assigned to this asset" text. */
  message?: string
}
...
export function usePmRules() {
  ...
  /**
   * Rule ids already assigned to an asset, mapped to their assignment's
   * is_active flag. The backend rejects a second assignment of the same
   * template with 409 whether the existing one is active or not, so the picker
   * must exclude both — and the flag lets it say which case it is.
   */
  async function loadAssignedRuleStates(assetId: number): Promise<Map<number, boolean>> {
    try {
      const all = await fetchList<AssetPmAssignment>(`/assets/${assetId}/pm-assignments`, {
        is_active: 'all',
      })
      return new Map(all.map((a) => [a.pm_rule_id, a.is_active]))
    } catch {
      return new Map()
    }
  }

  async function assignRule(assetId: number, pmRuleId: number): Promise<AssignRuleResult> {
    saving.value = true
    validationErrors.value = null
    try {
