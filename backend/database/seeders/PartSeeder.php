...
class PartSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($this->parts() as $code => $part) {
            ...
            Part::firstOrCreate(
                ...
                array_merge($part, [
                    'erp_part_code' => $code,
                    'maintenance_category_id' => $this->maintenanceCategoryId($categoryName),
                ]),
            );
        }
    }
    ...
    /**
     * Resolve (creating on first use) the controlled Maintenance Category for a
     * seeded part.
     *
     * Production categories arrive only through the controlled workbook import;
     * seeding them here is development fixture data so the Category/Size
     * compatibility rule has something to match on before LDC supplies the
     * cleaned workbooks.
     */
    private function maintenanceCategoryId(string $name): int
    {
        return MaintenanceCategory::firstOrCreate(
