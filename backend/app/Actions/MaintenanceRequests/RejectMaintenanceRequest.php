...
class RejectMaintenanceRequest
{
    public function execute(
        ...
        return DB::transaction(function () use ($maintenanceRequest, $rejectedByUserId, $reason, $suppressedUntilDate, $suppressedUntilReading) {
            ...
            $this->notifyRequester($locked->fresh(), $reason);

            return $locked->fresh();
        });
    }

    private function notifyRequester(MaintenanceRequest $mr, string $reason): void
    {
        $mr->load('asset', 'createdBy');

        $requesterEmail = $mr->createdBy?->email;
        ...
        Notification::route('account_email', 'workflow@atms')
            ->notify(new MaintenanceRequestRejectedNotification(
