...
class AssignWorkOrder
{
    public function execute(WorkOrder $workOrder, int $assignToUserId, int $assignedByUserId): WorkOrder
    {
        ...
        return DB::transaction(function () use ($workOrder, $assignToUserId, $assignedByUserId) {
            ...
            $this->notifyAssignee($locked->fresh(), $assignee);

            return $locked->fresh();
        });
    }

    private function notifyAssignee(WorkOrder $workOrder, User $assignee): void
    {
        if (! $assignee->email) {
            return;
        }
        ...
        Notification::route('account_email', 'workflow@atms')
            ->notify(new WorkOrderAssignedNotification(
                technicianEmail: $assignee->email,
