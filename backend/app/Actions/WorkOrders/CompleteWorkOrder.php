...
class CompleteWorkOrder
{
    public function execute(WorkOrder $workOrder, int $completedByUserId, ?string $completionNotes = null): WorkOrder
    {
        return DB::transaction(function () use ($workOrder, $completedByUserId, $completionNotes) {
            ...
            $this->notifyManagers($locked->fresh(), $completedByUserId);

            return $locked->fresh();
        });
    }

    private function notifyManagers(WorkOrder $workOrder, int $completedByUserId): void
    {
        ...
        Notification::route('account_email', 'workflow@atms')
            ->notify(new WorkOrderCompletedNotification(
                managerEmails: $managerEmails,
                woNumber: $workOrder->number,
                assetName: $workOrder->asset?->name ?? 'Unknown asset',
                technicianName: $completer?->name ?? 'Unknown',
