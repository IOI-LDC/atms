...
class WorkOrderController extends Controller
{
    ...
    public function setAssetStatus(Request $request, WorkOrder $workOrder, SetWorkOrderAssetStatus $action): JsonResponse
    {
        ...
        $resource = new AssetResource($workOrder->asset->fresh()->load(['currentLocation', 'maintenanceCategory']));
    ...
    /**
     * Save a set of field values in one atomic request — the write path behind
     * the checklist's explicit Save. Kept alongside the single-field PATCH,
     * which other consumers still use.
     */
    public function bulkUpdateFormFields(Request $request, WorkOrder $workOrder, BulkUpdateWorkOrderFormFieldValues $action): JsonResponse
    {
        Gate::authorize('updateExecution', $workOrder);

        $validated = $request->validate([
            'fields' => ['required', 'array', 'min:1', 'max:200'],
            // `distinct` rejects the same field twice in one batch, which would
            // otherwise make the applied order decide the stored value.
            'fields.*.id' => ['required', 'integer', 'distinct'],
            'fields.*.pre_value' => ['sometimes', 'nullable', 'string'],
            'fields.*.post_value' => ['sometimes', 'nullable', 'string'],
            'fields.*.notes' => ['sometimes', 'nullable', 'string'],
        ]);

        try {
            $form = $action->execute($workOrder, $validated['fields'], $request->user()->id);
