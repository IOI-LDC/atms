...
class MtbfReportTest extends TestCase
{
    ...
    public function test_legacy_category_group_by_is_rejected(): void
    {
        $this->actingAs($this->admin)
            ->getJson('/api/reports/mtbf?group_by=category')
            ->assertStatus(422);
    }
    ...
    public function test_groups_by_maintenance_category_code(): void
    {
        ...
        $this->assertCount(1, $json['items']);
        $this->assertSame('MUD_MOTOR', $json['items'][0]['group_key']);
        $this->assertSame('Mud Motor', $json['items'][0]['group_label']);
    }

    public function test_same_name_categories_with_different_codes_remain_separate(): void
    {
        $catA = MaintenanceCategory::factory()->create(['code' => 'MOTOR_A', 'name' => 'Motor']);
        $catB = MaintenanceCategory::factory()->create(['code' => 'MOTOR_B', 'name' => 'Motor']);
        $assetA = $this->createAsset(['maintenance_category_id' => $catA->id]);
        $assetB = $this->createAsset(['maintenance_category_id' => $catB->id]);
        $this->createFailure($assetA, 'MR-1');
        $this->createFailure($assetB, 'MR-2');

        $json = $this->actingAs($this->admin)
            ->getJson('/api/reports/mtbf?group_by=maintenance_category')->json();

        $keys = array_column($json['items'], 'group_key');
        $this->assertContains('MOTOR_A', $keys);
        $this->assertContains('MOTOR_B', $keys);
    ...
    public function test_groups_by_size_with_canonical_key_and_og_label(): void
    {
        ...
        $json = $this->actingAs($this->admin)
            ->getJson('/api/reports/mtbf?group_by=size')->json();
    ...
    public function test_equivalent_sizes_group_together(): void
    {
        $assetA = $this->createAsset(['size_inches' => '6.75']);
        $assetB = $this->createAsset(['size_inches' => '6 3/4"']);
        $this->createFailure($assetA, 'MR-1');
        $this->createFailure($assetB, 'MR-2');
    ...
    public function test_groups_by_asset_class(): void
    {
        $classified = $this->createAsset(['fa_subclass_code' => 'MWD']);
        $unclassified = $this->createAsset(['fa_subclass_code' => null]);
        $this->createFailure($classified, 'MR-1');
        $this->createFailure($unclassified, 'MR-2');

        $json = $this->actingAs($this->admin)
            ->getJson('/api/reports/mtbf?group_by=asset_class')->json();

        $this->assertCount(2, $json['items']);
        $byKey = collect($json['items'])->keyBy('group_key');
        $this->assertSame('MWD', $byKey['MWD']['group_label']);
        $this->assertSame('unclassified', $byKey['unclassified']['group_key']);
        $this->assertSame('Unclassified', $byKey['unclassified']['group_label']);
    }

    public function test_asset_grouping_includes_identity(): void
    {
        $category = MaintenanceCategory::factory()->create([
            'code' => 'JAR',
            'name' => 'Jar',
        ...
        $json = $this->actingAs($this->admin)
            ->getJson('/api/reports/mtbf?group_by=asset')->json();

        $item = $json['items'][0];
        $this->assertSame($asset->id, $item['group_key']);
        $this->assertArrayHasKey('asset', $item);
        $this->assertSame($asset->id, $item['asset']['id']);
        $this->assertSame('Motor 12', $item['asset']['name']);
        $this->assertSame('SN-999', $item['asset']['serial_number']);
        $this->assertSame('9 5/8"', $item['asset']['size']);
        $this->assertSame('9.62500', $item['asset']['size_inches']);
        $this->assertSame('JAR', $item['asset']['maintenance_category']['code']);
    }
