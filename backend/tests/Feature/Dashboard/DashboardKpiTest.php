...
class DashboardKpiTest extends TestCase
{
    ...
    public function test_relocated_assets_embed_full_asset_identity(): void
    {
        $admin = $this->createUser(RoleCode::ADMINISTRATOR);
        ...
        AssetLocationHistory::create([
            'asset_id' => $asset->id, 'from_location_id' => $from->id, 'to_location_id' => $to->id,
            'effective_at' => now()->subDay(), 'reason' => 'Move', 'changed_by_user_id' => $admin->id,
        ]);

        $relocated = $this->actingAs($admin)->getJson('/api/dashboard/kpis')->json('recently_relocated_assets');

        $this->assertCount(1, $relocated);
        $assetPayload = $relocated[0]['asset'];
        $this->assertSame($asset->id, $assetPayload['id']);
        $this->assertSame('Mud Motor Alpha', $assetPayload['name']);
        $this->assertSame('TAG-001', $assetPayload['asset_tag']);
        $this->assertSame('SN-001', $assetPayload['serial_number']);
        $this->assertSame('6 3/4"', $assetPayload['size']);
        $this->assertSame('6.75000', $assetPayload['size_inches']);
        $this->assertSame('MUD_MOTOR', $assetPayload['maintenance_category']['code']);
        $this->assertSame('Mud Motor', $assetPayload['maintenance_category']['name']);
        $this->assertArrayNotHasKey('erp_asset_code', $assetPayload);
    }
