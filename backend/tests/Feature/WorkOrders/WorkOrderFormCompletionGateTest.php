...
class WorkOrderFormCompletionGateTest extends TestCase
{
    ...
    /**
     * Product decision (2026-07-28): an untouched required boolean blocks
     * Complete — it is NOT an implicit "No". A check nobody answered must not
     * be recorded as one deliberately answered, so null stays unfilled while an
     * explicit '0' passes the gate.
     */
    public function test_untouched_required_boolean_blocks_completion(): void
    {
        $wo = $this->buildFormWorkOrder();
        $form = $wo->fresh()->load('workOrderForm.fields')->workOrderForm;
        ...
        // Everything filled except the boolean, which is left untouched (null).
        $this->actingAs($this->tech)->patchJson("/api/work-orders/{$wo->id}/form/fields/{$numeric->id}", [
            'pre_value' => '100',
            'post_value' => '120',
        ])->assertOk();
        ...
        $response = $this->actingAs($this->tech)->postJson("/api/work-orders/{$wo->id}/complete", [
            'completion_notes' => 'Boolean never answered',
        ]);

        $response->assertStatus(422);
        $this->assertTrue(collect($response->json('missing'))->contains(fn ($m) => $m['label'] === 'Cleaned?'));
        $this->assertEquals(WorkOrderStatus::IN_PROGRESS, $wo->fresh()->status);

        // An explicit "No" is a real answer and clears the gate.
        $this->actingAs($this->tech)->patchJson("/api/work-orders/{$wo->id}/form/fields/{$boolean->id}", [
            'post_value' => '0',
        ])->assertOk();

        $this->actingAs($this->tech)->postJson("/api/work-orders/{$wo->id}/complete", [
            'completion_notes' => 'Answered No',
        ])->assertOk();
