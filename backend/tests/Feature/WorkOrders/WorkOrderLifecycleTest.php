...
class WorkOrderLifecycleTest extends TestCase
{
    ...
    /** The work order list renders the serial number as its own column. */
    public function test_asset_embed_exposes_the_serial_number(): void
    {
        $requester = $this->createUser(RoleCode::REQUESTER);
        $manager = $this->createUser(RoleCode::MAINTENANCE_MANAGER);
        $asset = Asset::create([
        ...
        $this->createApprovedWorkOrder($requester, $manager, $asset);

        $this->actingAs($manager)
            ->getJson('/api/work-orders')
            ->assertOk()
            ->assertJsonPath('data.0.asset.serial_number', 'WZL-160-04');
    }
