...
class LocationWorkflowTest extends TestCase
{
    ...
    public function test_location_code_is_required(): void
    {
        $admin = $this->createUser(RoleCode::ADMINISTRATOR);

        $this->actingAs($admin)->postJson('/api/admin/locations', [
            'name' => 'No Code',
            'type' => 'warehouse',
        ])->assertUnprocessable()->assertJsonValidationErrors('code');
    }

    public function test_location_code_must_be_two_or_three_characters(): void
    {
        $admin = $this->createUser(RoleCode::ADMINISTRATOR);

        // Too short (1 char) and too long (4 chars) are rejected.
        $this->actingAs($admin)->postJson('/api/admin/locations', [
            'name' => 'One Char',
            'type' => 'warehouse',
            'code' => 'A',
        ...
        // 2 and 3 characters are accepted.
        $this->actingAs($admin)->postJson('/api/admin/locations', [
            'name' => 'Two Chars',
            'type' => 'warehouse',
            'code' => 'AB',
    ...
    public function test_location_code_must_be_unique(): void
    {
        ...
        Location::create(['name' => 'Workshop', 'type' => 'workshop', 'code' => 'WS']);
    ...
    public function test_location_code_uniqueness_ignores_self_on_update(): void
    {
        $admin = $this->createUser(RoleCode::ADMINISTRATOR);

        $location = Location::create(['name' => 'Workshop', 'type' => 'workshop', 'code' => 'WS']);

        // Keeping the same code while editing other fields is allowed.
        $this->actingAs($admin)->patchJson("/api/admin/locations/{$location->id}", [
            'name' => 'Workshop Renamed',
            'code' => 'WS',
        ])->assertOk();

        // Adopting another location's code is rejected.
        Location::create(['name' => 'Main Yard', 'type' => 'yard', 'code' => 'MY']);
