...
| `maintenance_manager` | Maintenance Manager | Review/approve MRs, manage WOs, PM rules, trigger ERP sync |
...
Copy these into `frontend/src/types/`. They model the **union** of all
...
PM MRs are generated by the scheduler (`EvaluatePmRulesJob`, daily) or on demand:

```
① Manager creates a PM rule
   POST /api/pm-rules   body: { asset_id, name, trigger_type, interval_days?, interval_reading?, usage_reading_type_id? }
        (asset must be ERP-linked, else 422)

② Evaluation (automatic daily, or manual)
   POST /api/pm-rules/{rule}/evaluate     (single rule)
   POST /api/pm-rules/evaluate            (all active rules)
        → 201 { message:"PM request generated.", data: MaintenanceRequest }  when due
        → 200 { message:"PM rule is not due." }                              when not due

③ The generated MR follows the same approve → WO → close flow as §6.1 ②–⑤
④ On WO closure, the PM rule baseline (last_triggered_date / _reading) is updated
```

**Deactivate / reactivate** (presented as `Deactivate`, never `Delete`):

```
POST /api/pm-rules/{rule}/deactivate     (blocked → 409 if an active chain exists)
POST /api/pm-rules/{rule}/reactivate     (409 if already active)
```
...
| Method | Endpoint | Role | Sort | Filters |
|---|---|---|---|---|
| GET | `/api/pm-rules` | Any | name, created_at, is_active | is_active, asset_id, trigger_type |
| POST | `/api/pm-rules` | Admin/Mgr | — | asset must be ERP-linked |
| GET | `/api/pm-rules/{rule}` | Any | — | — |
| PATCH | `/api/pm-rules/{rule}` | Admin/Mgr | — | name, description, interval_days?, interval_reading? |
| POST | `/api/pm-rules/{rule}/deactivate` | Admin/Mgr | — | 409 if active chain |
| POST | `/api/pm-rules/{rule}/reactivate` | Admin/Mgr | — | 409 if already active |
| POST | `/api/pm-rules/{rule}/evaluate` | Admin/Mgr | — | 201 if due, 200 if not |
| POST | `/api/pm-rules/evaluate` | Admin/Mgr | — | evaluates all active |
...
2. **Some mutation endpoints return the raw model, not a resource.** For example,
   `POST /api/maintenance-requests/corrective`, `POST /api/pm-rules`, and
   attachment uploads return `{ data: <model attributes> }` rather than a
   resource-shaped object. After a successful create, **re-fetch via the GET
   show endpoint** if you need the fully resource-shaped/role-filtered object
   (e.g. to render `asset`, `created_by`, `download_url`).
...
7. **PM rules require an ERP-linked asset** (`erp_asset_id` non-null). Creating a
   rule on a non-ERP asset returns `422`.
