...
| GET / POST | `/work-orders/{workOrder}/attachments` | WO attachment list/upload. Uploads are allowed through **completed** — the technician finishes the job, then files whatever paperwork exists — and are **403** once the work order is closed or cancelled. Attachments are **optional at close** (2026-08-30). Assigned technician, Manager or Admin. No mime allowlist: PDFs, spreadsheets and photographs are all accepted. |
