...
| `maintenance_manager` | Maintenance Manager | Review/approve MRs, manage WOs, view PM rules, run evaluation, trigger ERP sync |
...
demand. **Rule configuration (create/edit/deactivate/reactivate) is
Administrator-only.** Maintenance Manager can view rules and run manual
evaluation.

```
① Admin creates a PM rule
   POST /api/pm-rules   body: { asset_id, name, maintenance_level?, trigger_type,
...
② Evaluation (automatic daily, or manual)
   POST /api/pm-rules/{rule}/evaluate     (single rule – Admin/Manager)
   POST /api/pm-rules/evaluate            (all active rules – Admin/Manager)
...
④ On WO closure, the PM rule baseline (last_triggered_date / _reading) is
   updated. **Cumulative maintenance:** if the PM rule has a standard L1–L4
   level, closing a higher-level WO also resets all active lower-level PM
   rules on the same asset (e.g. closing L3 resets L1 and L2 baselines).
   Custom free-text levels are independent.
```

**Deactivate / reactivate** (Admin only; presented as `Deactivate`, never
`Delete`):

```
POST /api/pm-rules/{rule}/deactivate     (Admin only; blocked → 409 if an active chain exists)
...
| Method | Endpoint | Role | Sort | Filters |
|---|---|---|---|---|
| GET | `/api/pm-rules` | Admin/Mgr | name, created_at, is_active | is_active, asset_id, trigger_type |
| POST | `/api/pm-rules` | **Admin** | — | body: asset_id, name, maintenance_level?, trigger_type, interval_days?, interval_reading?, usage_reading_type_id? |
...
| POST | `/api/pm-rules/{rule}/deactivate` | **Admin** | — | 409 if active chain |
