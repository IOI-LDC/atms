...
① Manager creates a PM rule
   POST /api/pm-rules   body: { asset_id, name, trigger_type, interval_days?, interval_reading?, usage_reading_type_id? }
        (asset must be an ATMS-managed asset, else 422)
...
| POST | `/api/pm-rules` | Admin/Mgr | — | asset must be an ATMS-managed asset |
...
7. **PM rules require an ATMS-managed asset.** Creating a
   rule on a non-ATMS asset returns `422`.
