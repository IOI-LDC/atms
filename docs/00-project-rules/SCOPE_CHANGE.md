...
| 22 | Asset Maintenance Status | `enrolled` (standalone / `installed` / `ready`) and `withdrawn` (`lih`, `dbr`, `disposed`, `scrapped`, `other`). Independent of ERP financial treatment. Values renamed 2026-07-02 from `Active`/`Inactive` to avoid the `operational_status='active'` collision. | Client needed clearer asset lifecycle states beyond simple active/inactive. |
...
| Scope item | Detail |
|---|---|
| Asset registry | Full asset CRUD with maintenance status (`enrolled`/`withdrawn` + sub-statuses), asset tags (`L-BBB-CCC-XXXX`), operational fields |
