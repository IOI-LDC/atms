...
| 2026-07-11 | **Email transport decision confirmed: Microsoft Graph `sendMail` only; Power Automate retired.** Phase 1 email scope is activation + password reset only; operational MR/WO emails are future scope. Azure app provisioned + `Mail.Send` consented + probe passed (HTTP 202). **Not built yet:** Graph implementation behind `AccountEmailTransport`, queue serialization + 429 retry, `GRAPH_*` config/binding, tests, and removal of the legacy Power Automate class/config/binding/tests. Pre-release: final frontend URL, Application Access Policy, real user emails, production credential, queue-worker verification. See `docs/03-backend/NOTIFICATIONS.md`. | Backend (account-email transport build) | ⚠️ Unacknowledged |
